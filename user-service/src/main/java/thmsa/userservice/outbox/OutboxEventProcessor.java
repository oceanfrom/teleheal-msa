package thmsa.userservice.outbox;
...
public class OutboxEventProcessor {
    ...
    @Value("${user.topic.dlq.name}")
    private String USER_TOPIC_DLQ_NAME;
    ...
    public void processOutboxEvents() {
        ...
        for (OutboxEvent event : events) {
            try {
                kafkaTemplate.send(USER_TOPIC_NAME, event.getPayload()).get();
                event.setSent(true);
    ...
    private void sendToDeadLetterQueue(OutboxEvent event) {
        try {
            kafkaTemplate.send(USER_TOPIC_DLQ_NAME, event.getPayload());
        } catch (Exception ex) {
